...
const pages: Record<string, { text: string; illustration_description: string }[]> = {
  'luna-star-garden': [
...
async function main() {
  for (const book of books) {
    ...
    const bookPages = pages[book.id];
    for (let i = 0; i < bookPages.length; i++) {
      await prisma.page.upsert({
        where: { book_id_page_number: { book_id: book.id, page_number: i + 1 } },
        update: {
          text: bookPages[i].text,
          illustration_description: bookPages[i].illustration_description,
        },
        create: {
          book_id: book.id,
          page_number: i + 1,
  ...
  console.log(`Seeded ${books.length} books with pages`);
}
